...
{
  "lot_number": "WF-KR001-251110D-001",
  "line_id": "KR001",
  "process_id": "PROC-002",
  "worker_id": "W001"
}
...
```http
GET /api/lots/WF-KR001-251110D-001
...
| line_id | string | Y | 생산 라인 ID (KR01, KR02 등) - 국가코드 2자리 + 라인번호 2자리 |
...
## 3.4.2 WIP Item 관리 인터페이스

**개요:** WIP (Work-In-Progress) 아이템의 생성, 공정 진행, 상태 관리 및 Serial 전환을 위한 인터페이스
...
### 3.4.2.1 WIP Item 목록 조회

**API 엔드포인트:** `GET /api/v1/wip-items`

**쿼리 파라미터:**

| 파라미터 | 타입 | 필수 | 기본값 | 설명 |
|---------|------|------|--------|------|
| lot_id | integer | N | - | LOT ID로 필터링 |
| status | string | N | - | WIP 상태로 필터링 (CREATED, IN_PROGRESS, COMPLETED, FAILED, CONVERTED) |
| skip | integer | N | 0 | 페이지네이션 offset |
| limit | integer | N | 100 | 페이지네이션 limit (최대 1000) |

**요청 예시:**

```
GET /api/v1/wip-items?lot_id=1&status=IN_PROGRESS&skip=0&limit=100
...
### 3.4.2.2 WIP Item 상세 조회

**API 엔드포인트:** `GET /api/v1/wip-items/{wip_id}`

**경로 파라미터:**

| 파라미터 | 타입 | 필수 | 설명 |
|---------|------|------|------|
| wip_id | integer | Y | WIP item 식별자 (DB ID) |

**요청 예시:**

```
GET /api/v1/wip-items/123
Authorization: Bearer {JWT_TOKEN}
```
...
**API 엔드포인트:** `POST /api/v1/wip-items/{wip_id}/start-process`
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| process_id | integer | Y | 공정 ID (1~6) |
| operator_id | integer | Y | 작업자 ID |
| equipment_id | integer | N | 설비 ID |
| started_at | string | N | 시작 시간 (ISO 8601, 기본값: 현재 시간) |
...
**비즈니스 규칙:**
- **BR-003**: 이전 공정이 PASS 상태여야 함 (공정 1 제외)
- WIP 상태가 CREATED 또는 IN_PROGRESS여야 함

**에러 응답:**
- `404 Not Found`: WIP item이 존재하지 않음
- `400 Bad Request`: 이전 공정이 완료되지 않음

### 3.4.2.4 WIP 공정 완료

**API 엔드포인트:** `POST /api/v1/wip-items/{wip_id}/complete-process`
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| result | string | Y | 공정 결과 (PASS, FAIL, REWORK) |
| measurements | object | N | 측정 데이터 (JSON) |
| defects | array | N | 불량 목록 (FAIL인 경우 필수) |
| notes | string | N | 비고 |
| completed_at | string | N | 완료 시간 (ISO 8601, 기본값: 현재 시간) |
...
**비즈니스 규칙:**
- **BR-004**: 동일 공정에 중복 PASS 결과 불가
- FAIL 결과 시 defects 배열 필수
- REWORK 결과 시 WIP 상태는 IN_PROGRESS 유지

### 3.4.2.5 WIP → Serial 전환

**API 엔드포인트:** `POST /api/v1/wip-items/{wip_id}/convert-to-serial`
...
```json
{
  ...
  "wip_item": {
    ...
    "status": "CONVERTED",
    "serial_id": 789,
    "converted_at": "2025-01-21T11:00:00Z"
  }
}
```

**비즈니스 규칙:**
- **BR-005**: 모든 공정 1~6이 PASS 상태여야 함
- WIP 상태가 COMPLETED여야 함
- Serial Number 생성 규칙: `{lot_number}{sequence:04d}`

**에러 응답:**
- `400 Bad Request`: 공정 1~6 미완료
- `409 Conflict`: 이미 Serial로 전환됨

### 3.4.2.6 WIP 바코드 생성

**API 엔드포인트:** `GET /api/v1/wip-items/barcode/{wip_id}`

**경로 파라미터:**

| 파라미터 | 타입 | 필수 | 설명 |
|---------|------|------|------|
| wip_id | string | Y | WIP ID (예: WIP-KR01PSA2511-001) |

**쿼리 파라미터:**

| 파라미터 | 타입 | 필수 | 기본값 | 설명 |
|---------|------|------|--------|------|
| barcode_type | string | N | code128 | 바코드 타입 (code128, qr) |

**요청 예시:**

```
GET /api/v1/wip-items/barcode/WIP-KR01PSA2511-001?barcode_type=code128
Authorization: Bearer {JWT_TOKEN}
```
...
**API 엔드포인트:** `GET /api/v1/wip-items/statistics`
...
|---------|------|------|------|
| lot_id | integer | N | LOT ID로 필터링 |
| process_id | integer | N | 공정 ID로 필터링 |

**요청 예시:**

```
GET /api/v1/wip-items/statistics?lot_id=1&process_id=2
Authorization: Bearer {JWT_TOKEN}
```
...
| 필드 | 타입 | 설명 |
|------|------|------|
| total | integer | 전체 WIP 개수 |
| created | integer | CREATED 상태 개수 |
| in_progress | integer | IN_PROGRESS 상태 개수 |
| completed | integer | COMPLETED 상태 개수 |
| failed | integer | FAILED 상태 개수 |
| converted | integer | CONVERTED 상태 개수 (Serial 전환 완료) |
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| lot_number | string | Y | LOT 번호 (예: KR01WFO2511001, 상세는 [2.6.1 LOT 번호 체계](../02-product-process.md#261-lot-번호-체계) 참조) |
| line_id | string | Y | 생산 라인 ID (KR01, KR02 등) - 국가코드 2자리 + 라인번호 2자리 |
| process_id | string | Y | 공정 ID (PROC-001 ~ PROC-008) |
| process_name | string | Y | 공정명 (한글) |
...
{
  "process_data": {
    "lot_number_engraved": "WF-KR001-251110D-001",
...
- 예시: `KR01WFO2511001_PROC-003_20250110090000.json`
...
| | overall_result | string | "PASS" or "FAIL" | **필수** 전체 검사 결과 |
| **라벨 프린팅** | serial_number | string | 형식: WF-{LINE}-YYMMDDX-nnn-nnnn | 생성된 시리얼 번호 |
...
{
  "label_type": "SERIAL",
  "serial_number": "KR01WFO25110010001",
  "lot_number": "KR01WFO2511001",
  "product_model": "Withforce",
  "created_date": "2025-01-10",
  "printer_id": "PRINTER-07"
    ...
    "alerts": [
      {
        ...
        "severity": "high",
        "title": "불량 발생",
        "message": "LOT-KR01WFO2511001, 공정 3 (센서 검사)에서 불량 발생",
        "related_lot": "KR01WFO2511001",
      ...
      },
      {
        ...
        "severity": "medium",
        "title": "공정 지연",
        "message": "LOT-KR01WFO2511002, 공정 5 (로봇 조립)이 예상 시간 초과",
        "related_lot": "KR01WFO2511002",
