...
**처리 결과:**
- LOT 번호 자동 발급 (예: `KR01PSA2511001`, 상세 체계는 [2.6.1 LOT 번호 체계](../02-product-process.md#261-lot-번호-체계) 참조)
- 생성 시 상태: CREATED
- LOT 정보 데이터베이스 저장
...
- **설명**: 공정 1 시작 시 LOT에서 WIP Item을 생성한다
- **입력**: LOT ID, 생성 수량 (1~100)
- **처리**:
  - WIP ID 생성: `WIP-{LOT번호}-{순번:03d}`
  - 순번은 001부터 순차 증가
  - LOT당 최대 100개까지 생성 가능
  - 상태: CREATED
- **출력**: WIP Item 목록 (id, wip_id, lot_id, sequence_in_lot)
- **제약조건**:
  - LOT 상태가 CREATED여야 함 (BR-001)
  - WIP 생성 후 LOT 상태가 IN_PROGRESS로 변경됨 (BR-002)
  - WIP ID는 전역적으로 고유해야 함 (BR-007)
  - LOT당 최대 100개 제한 (BR-006)

**FR-WIP-002: WIP Item 추적**
- **설명**: 공정 2~6에서 WIP ID를 스캔하여 제품을 추적한다
- **입력**: WIP ID (바코드 스캔 또는 수동 입력)
- **처리**:
  - WIP ID 유효성 검증 (형식, 존재 여부)
  - WIP 상태 확인 (CREATED, IN_PROGRESS 가능)
  - 현재 공정 정보 조회
- **출력**: WIP Item 상세 정보
- **제약조건**:
  - WIP ID 형식: `^WIP-([A-Z0-9]{11})-(\d{3})$`
  - WIP가 존재하지 않으면 404 오류
  - WIP 상태가 CONVERTED 또는 FAILED이면 처리 불가

**FR-WIP-003: WIP → Serial 전환**
- **설명**: 공정 7 시작 시 WIP를 Serial Number로 전환한다
- **입력**: WIP ID, 작업자 ID
- **처리**:
  - 모든 공정 1~6 PASS 확인 (BR-005)
  - Serial Number 생성: `{LOT번호}{순번:04d}`
  - WIP 순번을 Serial 순번으로 재사용
  - WIP 상태: CONVERTED
  - Serial 상태: CREATED
  - converted_at 타임스탬프 기록
- **출력**: Serial Number 정보
- **제약조건**:
  - 공정 1~6이 모두 PASS여야 함
...
- **설명**: 공정 실패 시 REWORK를 통해 재작업을 허용한다
- **입력**: WIP ID, 공정 ID, 결과 (REWORK)
- **처리**:
  - WIP 상태: IN_PROGRESS 유지 (FAILED 아님)
  - current_process_id: null (재시작 가능)
  - REWORK 횟수: 무제한 (비즈니스 정책)
- **출력**: WIP 상태 업데이트
- **제약조건**:
  - REWORK 후 동일 공정 재실행 가능
  - REWORK는 PASS 또는 FAIL로 최종 완료 필요
...
#### 3.2.3.4 WIP-Serial 전환 비즈니스 로직

**전환 조건**:

1. WIP 상태가 COMPLETED일 것
2. 모든 공정 1~6이 PASS 결과를 가질 것
3. 공정 7 시작 시점에 전환 실행
...
[Serial Number 생성]
    ├─ 형식: {LOT번호}{순번:04d}
    ├─ 순번: WIP sequence_in_lot 재사용
    └─ 예: KR01PSA25110001
    ↓
[WIP 업데이트]
    ├─ status: CONVERTED
    ├─ serial_id: 생성된 Serial ID
    └─ converted_at: 현재 시각
    ↓
[Serial 생성 완료]
```
...
- 공정 1~6 중 FAIL 또는 미완료: 400 Bad Request
- WIP 이미 전환됨: 409 Conflict
- WIP 상태가 COMPLETED 아님: 400 Bad Request

**데이터 일관성**:

- 트랜잭션 내에서 WIP 업데이트 + Serial 생성
- Serial 생성 실패 시 WIP 상태 롤백
- wip_items.serial_id는 serials.id를 참조 (FK)
...
- 라인 코드 (예: KR01, KR02) - 국가코드 2자리 + 라인번호 2자리
- 라인명 (한글/영문)
- 위치 정보
- 시프트당 생산 용량
- 활성화 상태
