...
1. [프로젝트 개요](#1-프로젝트-개요)
2. [제품 및 공정 현황 (AS-IS)](#2-제품-및-공정-현황-as-is)
3. [생산 워크플로우 (TO-BE)](#3-생산-워크플로우-to-be)
4. [시스템 아키텍처](#4-시스템-아키텍처)
5. [LOT 및 시리얼 번호 체계](#5-lot-및-시리얼-번호-체계)
6. [데이터베이스 설계](#6-데이터베이스-설계)
7. [API 명세](#7-api-명세)
8. [보안 및 인증](#8-보안-및-인증)
9. [에러 처리 및 복구](#9-에러-처리-및-복구)
10. [백업 및 재해복구](#10-백업-및-재해복구)
11. [개발 계획](#11-개발-계획)
12. [투자 계획](#12-투자-계획)
13. [부록](#13-부록)
...
**제품명:** F2X Wearable Robot

**제품 유형:** 산업용/농업용 허리 보조 로봇 (Wearable Exoskeleton)

**제품 목적:**
- 산업 현장 및 농업 작업 시 작업자의 허리 부담 경감
- 반복적인 허리 굽힘 동작 지원
- 근골격계 질환 예방

**주요 구성:**
- **LMA 구동기** (Linear Muscle Actuator): 로봇의 핵심 구동 장치
- **메인 프레임**: 로봇의 골격 구조
- **제어 보드**: 펌웨어 기반 제어 시스템
- **센서**: 온도 센서, TOF 센서 등
...
| 용어 | 설명 |
|------|------|
| **LMA** | Linear Muscle Actuator (선형 근육 구동기)<br/>- 로봇의 핵심 구동 장치<br/>- SMA 스프링을 이용한 선형 운동 생성 |
| **SMA 스프링** | Shape Memory Alloy Spring (형상 기억 합금 스프링)<br/>- 온도에 따라 형상이 변하는 특수 합금 소재<br/>- LMA의 핵심 구동 부품 |
| **모선 케이블** | SMA 스프링의 부모 케이블 (Parent Cable)<br/>- SMA 스프링 제조 시 사용되는 원재료 |
| **EOL** | End of Line (최종 검사)<br/>- 조립 완료 후 전기적/기능적 검사 수행 |
| **TOF 센서** | Time of Flight Sensor<br/>- 거리 측정 센서<br/>- I2C 통신 방식 |
| **LOT** | 생산 단위 (일반적으로 일일/교대별 생산 수량)<br/>- 추적 관리의 기본 단위 |
| **시리얼 번호** | 개별 제품 고유 식별 번호<br/>- 완제품 단위 추적 번호 |
...
**근무 형태:** 주간 근무 (D: Day Shift), 야간 근무 (N: Night Shift)
...
**작업 대차 운영:**
- 1개 작업대차 = 1대의 로봇
- LOT 단위로 생산 (예: LOT-2025-001 = 100대 = 작업대차 100대)
- 각 작업대차에 LOT 바코드 라벨 부착
- 작업대차가 공정을 순차적으로 이동
...
**작업 내용:**
- 정해진 조립 순서에 따라 LMA 구동기 조립
- 작업자 수작업 조립

**품질 검사:**
- 검사 방법: 작업자 육안 검사
- 검사 항목: 조립 완성도, 외관 검사, 부품 누락 확인

**표준 작업시간:** 1시간 이내

**작업 장소:** 작업 PC #1 (192.168.1.101)
...
**마킹 내용:** LOT 번호 (예: FN-KR-251110D-001)

**마킹 위치:** LMA 구동기 표면 (작업자가 위치 결정)

**장비:** 레이저 마킹기 (Local App 제어)

**작업 내용:**
- 작업자가 마킹 위치 설정
- Local App을 통해 레이저 마킹기 제어
- LOT 번호 각인
...
**검사 장비:** 자동 검사 장비 (Local App 제어 및 데이터 수집)
...
3. **펌웨어 업로드**
   ...
   - 업로드 방식: Local App이 제어
   - 성공 판단: Local App이 자동 판단
...
**작업 장소:** 작업 PC #3 (192.168.1.103)

**수집 데이터:** (MES 구축 후)
- 온도 센서 측정값 (℃)
- TOF 센서 통신 상태 (Success/Fail)
- 펌웨어 버전
- 업로드 성공/실패 여부
- 검사 시간
- 작업자 ID
...
#### 2.4.4 공정 4: 로봇 조립 + 성능검사

**공정 목적:** LMA 구동기와 메인 프레임 조립 및 성능 검사
...
**공정 목적:** 시리얼 번호 생성 및 라벨 출력

**전공정 정합성 체크:**
- 모든 공정 (1~4) 착공 및 완공 확인
- 모든 공정 품질 검사 Pass 확인
- 정합성 확인 후에만 시리얼 번호 생성

**시리얼 번호 생성:**
- 생성 시점: 착공 API 호출 시 자동 생성
- 생성 방식: LOT 번호 기반 순차 번호
- 형식: `{LOT번호}-{순번}` (예: FN-KR-251110D-001-0001)
- 대차 1대당 시리얼 1개 생성

**라벨 출력:**
- 프린터: Zebra 라벨 프린터
- 출력 내용: 시리얼 번호 QR Code 또는 바코드
- 출력 매수: 1장 (대차 1대당)
- 라벨 부착: 작업자가 완성 로봇에 부착
...
- 어떤 펌웨어 버전이 설치되었는지 알 수 없음
- 불량 발생 시 동일 LOT 제품 식별 불가
...
**5. 외부 모니터링 불가**
- 관리자가 현장에 있어야만 현황 파악 가능
- 외부(본사, 재택)에서 생산 현황 확인 불가
...
3. ✅ **품질 데이터 수집**: 모든 측정 데이터 자동 수집 및 분석
4. ✅ **자동화**: 수작업 최소화, 자동 기록
5. ✅ **외부 접근**: 웹 대시보드를 통한 원격 모니터링
...
sequenceDiagram
    ...
    participant 작업자 as 현장 작업자
    participant 공정앱 as 외부 공정 앱

    Note over 관리자,MES: 1️⃣ LOT 생성
    관리자->>MES: LOT 생성 요청
    MES-->>관리자: LOT 번호 발급
    ...
    Note over 작업자,MES: 3️⃣ 공정 착공
    작업자->>작업자: LOT 바코드 스캔
    작업자->>MES: 착공 등록
    MES-->>작업자: 착공 확인

    Note over 공정앱,MES: 4️⃣ 공정 완공
    공정앱->>공정앱: 공정 작업 수행
    공정앱->>MES: 완공 데이터 전송
    MES-->>공정앱: 완공 확인

    Note over 작업자,MES: 5️⃣ 시리얼 발행 (Label Printing)
    작업자->>MES: 착공 + 시리얼 발행 요청
    MES-->>작업자: 시리얼 번호 생성<br/>라벨 출력

    Note over 관리자,MES: 6️⃣ 실시간 모니터링
    관리자->>MES: 생산 현황 조회
    MES-->>관리자: 실시간 현황 제공
```
...
1. **LOT 생성** (관리자) → 생산 계획 수립, LOT 번호 발급
2. **LOT 라벨 출력** (관리자) → 작업대차에 부착할 바코드 라벨
3. **공정 착공** (작업자) → 바코드 스캔으로 공정 시작 등록
4. **공정 완공** (공정 앱) → 작업 결과 데이터 전송
5. **시리얼 발행** (Label Printing) → 개별 제품 번호 생성 및 라벨 출력
6. **모니터링** (관리자) → 실시간 생산 현황 확인
...
**기능:**
- 제품 모델, 목표 수량, 교대조를 입력하여 LOT 생성
- 시스템이 LOT 번호를 자동 발급
- LOT 번호 형식: `FN-KR-251110D-001` (제품코드-공장-날짜교대-일련번호)
...
- 외부 공정 앱에서 작업 완료
- 공정 데이터를 MES로 전송
- MES에서 완공 처리 및 다음 공정 허용

**데이터 전송 방식:**
- JSON 파일 기반 통신 (외부 공정 앱 제약사항)

---

### 3.5 시리얼 번호 발행 (Label Printing 공정)

**수행 주체:** 현장 작업자

**수행 시점:** Label Printing 공정 착공 시

**기능:**
- LOT 스캔 시 자동으로 시리얼 번호 생성
- 시리얼 형식: `FN-KR-251110D-001-0001`
- 시리얼 라벨 자동 출력
- 작업자가 로봇에 라벨 부착

**처리 흐름:**
- 1대차 도착 → 스캔 → 시리얼 생성 (예: -0001)
- 2대차 도착 → 스캔 → 시리얼 생성 (예: -0002)
- 반복 (LOT 목표 수량까지)
...
graph TB
    subgraph Factory["🏭 생산 현장 (Factory Floor)"]
        PC1["공정 1-7<br/>작업 PC #1<br/><br/>프론트엔드 앱<br/>바코드 스캐너<br/>오프라인 큐"]
        PC2["공정 1-7<br/>작업 PC #2<br/><br/>프론트엔드 앱<br/>바코드 스캐너<br/>오프라인 큐"]
        PC3["공정 1-7<br/>작업 PC #3<br/><br/>프론트엔드 앱<br/>바코드 스캐너<br/>오프라인 큐"]
        PC4["공정 1-7<br/>작업 PC #4<br/><br/>프론트엔드 앱<br/>바코드 스캐너<br/>오프라인 큐"]
        PCN["..."]
    end
    ...
    subgraph Monitoring["📊 관리자 접속 (Monitoring)"]
        ...
        Web1 -.-> Dashboard
        Web2 -.-> Dashboard
   - 시퀀스 자동 증가: 1번째 → FN-KR-251110D-001-0001
4. 프린터가 라벨 1장 자동 출력
5. 작업자가 출력된 라벨을 로봇에 부착
6. 대차 #1 완공 → 다음 공정(포장)으로 이동

[대차 #2 도착]
7. 작업대차 #2가 Label Printing 공정에 도착
8. 작업자가 동일한 LOT 바코드 스캔 (LOT-2025-001)
9. 착공 API 호출 → 시리얼 1개 자동 생성
   - 시퀀스 자동 증가: 2번째 → FN-KR-251110D-001-0002
10. 프린터가 라벨 1장 자동 출력
11. 작업자가 라벨을 로봇에 부착
12. 대차 #2 완공 → 다음 공정으로 이동

... (대차 #3 ~ #100 반복)

[대차 #100 도착]
- 마지막 시리얼 생성: FN-KR-251110D-001-0100
- LOT-2025-001의 모든 제품 완료

[재출력 기능]
- 라벨 손상 또는 분실 시 사용
- 프론트엔드 앱에서 "재출력" 버튼 클릭
- 마지막 출력된 시리얼 번호의 라벨 재출력
- 예: FN-KR-251110D-001-0002 재출력
```
...
```
LOT-2025-001 (수량 100개) = 대차 100대

[대차 #1] FN-KR-251110D-001-0001
[대차 #2] FN-KR-251110D-001-0002
[대차 #3] FN-KR-251110D-001-0003
...
[대차 #100] FN-KR-251110D-001-0100
```

---

#### 착공 API (Label Printing 공정 특수 처리)

**엔드포인트:** `POST /api/v1/work/start`

**중요:** Label Printing 공정에서는 착공 시 자동으로 시리얼 번호 1개 생성

**요청 데이터:**

- `lot_number`: LOT 번호
- `process_id`: "PROC06" (Label Printing)
- `process_name`: "Label Printing"
- `worker_id`: 작업자 ID
- `worker_name`: 작업자 이름

**응답 데이터 (Label Printing 공정):**

- `success`: true
- `work_started`: true (착공 완료)
- `serial_generated`: true (시리얼 생성됨)
- `serial_number`: 생성된 시리얼 번호 (예: FN-KR-251110D-001-0024)
- `sequence`: 현재 시퀀스 (24)
- `total_quantity`: 전체 수량 (100)
- `remaining`: 남은 수량 (76)
- `print_command`: 프린터 출력 명령 (프린터 ID, 템플릿, 출력 데이터)
- `message`: "시리얼 번호 생성 완료. 라벨이 출력됩니다."

**응답 데이터 (일반 공정):**

- `success`: true
- `work_started`: true
- `serial_generated`: false (시리얼 생성 안 함)
- `message`: "착공 완료"

---

#### 프론트엔드 앱 UI (Label Printing 전용)

**Label Printing 공정은 프론트엔드 앱에서만 처리 (로컬 공정 앱 불필요)**
...
│  LOT 번호를 스캔하세요                   │
│                                         │
│  현재 LOT: LOT-2025-001 (23/100 완료)   │
│                                         │
│  ┌───────────────────────────────────┐ │
│  │   마지막 생성된 시리얼:           │ │
│  │   FN-KR-251110D-001-0023         │ │
│  │                                   │ │
│  │   ✅ 라벨 출력 완료               │ │
│  │   → 로봇에 부착하세요            │ │
│  └───────────────────────────────────┘ │
...
1. 대차 도착 → LOT 바코드 스캔
2. 착공 + 시리얼 생성 + 라벨 출력 (자동)
3. 화면에 생성된 시리얼 표시
4. 작업자가 라벨을 로봇에 부착
5. "완료" 버튼 클릭 → 완공 JSON 자동 생성
6. 다음 대차 대기
...
**라벨 디자인 및 포맷은 사용자가 추후 정의합니다.**

**기본 포함 항목 (권장):**

- 바코드 타입: QR Code 또는 Code128 (사용자 선택)
- 시리얼 번호 (필수)
- LOT 번호 (선택)
- 제품명 (선택)
- 생산 날짜 (선택)
...
- 다양한 라벨 템플릿 설정 가능
- ZPL, ESC/POS 등 프린터 명령어 커스터마이징 지원
- 라벨 디자인은 프로젝트 진행 중 확정
...
- Zebra, TSC, Godex 등 일반 산업용 바코드 프린터
- 가격대: 약 50-200만원
- 연결 방식: USB 또는 네트워크
...
- `serials` 테이블에 레코드 1개 생성
- 초기 상태: `status = 'PRINTED'`, `current_process = 'PROC06'`
- 생성 순서(sequence) 저장: 1, 2, 3, ..., 100

**완공 시:**

- LOT 상태 업데이트: `status = 'LABEL_PRINTED'`
- 모든 시리얼 번호가 생성되었음을 기록
...
```
1. 관리자가 Dashboard에서 LOT 생성
2. LOT 생성 완료 후 "LOT 라벨 출력" 버튼 활성화
3. 버튼 클릭 → 프린터 설정 확인
4. LOT 바코드 라벨 1장 출력
5. 작업자가 라벨을 작업대차에 부착
6. 대차가 첫 번째 공정(스프링 투입)으로 이동
```
...
- 라벨 크기: 70mm x 50mm (LOT 라벨은 시리얼 라벨보다 큼)
- **시리얼 번호 라벨 출력**: Section 2.3 참조 (Label Printing 공정에서 처리)
...
- **착공:** 프론트엔드 앱 포커스 → 바코드 스캔 → 즉시 Backend API 호출
- **완공:** 로컬 공정 앱 → JSON 파일 생성 → 프론트엔드 앱 자동 전송

---

#### 작업 흐름

**중요:** 각 공정에서는 **LOT 번호**를 스캔합니다 (시리얼 번호 아님)

- 시리얼 번호는 최종 공정 Pass 후 자동 생성됩니다
- 작업대차가 각 공정을 이동하며 LOT 단위로 작업합니다

```
[착공 단계]
1. 작업대차가 해당 공정에 도착
2. 작업자가 프론트엔드 앱 화면에 포커스
3. LOT 바코드 스캔 (예: LOT-2025-001) ← 키보드 타입 바코드 리더기
4. 프론트엔드 앱이 착공 API 호출 (POST /api/v1/work/start)
5. 서버 응답 확인
   - 성공: "✅ 착공 완료! 로컬 공정 앱에서 작업을 진행하세요"
   - 실패: "❌ 오류: 이전 공정 미완료" 등
...
[완공 단계]
8. 로컬 공정 앱이 완공 JSON 파일 자동 생성 (C:\neurohub_work\pending\)
9. 프론트엔드 앱이 파일 감지 → 자동으로 완공 API 전송
```
...
- 착공 API 호출 및 결과 표시
- 완공 JSON 파일 감시 (watchdog)
- 완공 API 전송 및 파일 이동 (success/failed)
...
**파일 예시:** `complete_20250110_143100_LOT-2025-001.json`
...
```json
{
  ...
  "complete_time": "2025-01-10T14:35:10.654321",
  "process_data": {
    "모선_lot": "MS-2025-100",
    "스프링_lot": "SP-2025-050",
    "투입_수량": 100
  },
  "timestamp": "2025-01-10T14:35:10.654321"
}
```
...
1. **네트워크 격리**: 로컬 공정 앱은 서버 통신 불필요
2. **역할 분리**: 착공/완공 로직과 공정 작업 로직 분리
3. **디버깅 용이**: JSON 파일로 이력 추적 가능
4. **오프라인 동작**: 네트워크 장애 시에도 JSON 파일 생성, 복구 시 자동 전송
5. **재시도 메커니즘**: 실패한 파일 자동 재전송 (5분 주기)
...
**향후 업그레이드:** 통신 방식 바코드 리더기

- 백그라운드 서비스가 스캔 자동 감지
- 앱 전환 없이 스캔만으로 착공/완공 처리 가능
...
### 3.7 Label Printing 공정 상세 설명

**특징:** 프론트엔드 앱에서 모든 처리 (착공 → 시리얼 생성 → 라벨 출력 → 완공)

**핵심 차이점:**

- ✅ 로컬 공정 앱 불필요 (프론트엔드 앱만 사용)
- ✅ 착공 시 자동으로 시리얼 번호 생성
- ✅ 대차 1대당 시리얼 1개 생성
- ✅ 완공 JSON 자동 생성
...
1. **LOT 바코드 스캔 처리**
   - Label Printing 공정 여부 확인
   - Label Printing인 경우: 착공 + 시리얼 생성 동시 처리
   - 일반 공정인 경우: 착공만 처리

2. **UI 표시 정보**
   - 현재 LOT 번호 및 진행 상황 (예: 23/100)
   - 생성된 시리얼 번호
   - 라벨 출력 상태
   - 완료 버튼

3. **완공 처리**
   - 완료 버튼 클릭 시 완공 JSON 파일 자동 생성
   - 생성된 시리얼 번호 포함
   - 파일 감시 프로세스가 자동으로 서버 전송

---

#### 백엔드 API 처리 로직

**착공 API 엔드포인트:** `POST /api/v1/work/start`

**처리 흐름:**

1. 요청 데이터 수신 (lot_number, process_id, worker_id 등)
2. LOT 정보 조회 및 검증
3. 착공 기록 생성
4. **Label Printing 공정인 경우 추가 처리:**
   - 이미 생성된 시리얼 개수 확인
   - 시퀀스 자동 증가 (1, 2, 3, ...)
   - 시리얼 번호 생성 (예: FN-KR-251110D-001-0024)
   - DB에 시리얼 저장
   - 프린터 출력 명령 생성하여 응답에 포함
5. 응답 반환
...
- `serial_number`: 생성된 시리얼
- `sequence`: 현재 시퀀스
- `total_quantity`: 전체 수량
- `remaining`: 남은 수량
- `print_command`: 프린터 출력 명령
...
**출력 트리거:**
- 착공 API 응답에 포함된 `print_command` 수신 시 자동 출력

**지원 프린터:**
- Zebra, TSC, Godex 등 산업용 바코드 프린터
- ZPL 또는 ESC/POS 명령어 사용

**출력 데이터:**
- 시리얼 번호 (QR Code 또는 Code128)
- LOT 번호
- 제품명
- 생산 날짜
...
1. "재출력" 버튼 제공
2. 버튼 클릭 시 마지막 출력된 시리얼 번호 조회
3. 재출력 API 호출

**백엔드 API:** `POST /api/v1/serials/{serial_number}/reprint`
...
```json
{
  "success": true,
  "data": {
    "serial_number": "FN-KR-251110D-001-0024",
    "reprint_count": 2,
    "print_command": "^XA^FO50,50^BY2^BCN,100,Y,N,N^FD..."
  }
}
```
...
- 재출력 횟수 제한 없음 (필요시 무제한 재출력 가능)
- 재출력 이력은 모두 audit_log에 기록
- 재출력 시에도 동일한 시리얼 번호 사용 (새로운 시리얼 생성 안 함)
...
#### 4.5.4 공정별 `process_specific_data` 예시
...
#### 6.2.2 제품 모델 마스터 (product_models)
...
**설명:** 시리얼 번호 생성 (Label Printing 공정 착공 시 자동 호출)

**특징:**

- Label Printing 공정에서 작업자가 LOT 바코드를 스캔하면 자동으로 1개씩 생성
- 대차 1대당 시리얼 1개 생성 방식
- 생성 즉시 라벨 프린터로 출력 명령 전달
...
| **Frontend 개발자** | 1-2명 | 프론트엔드 앱, React Dashboard 개발 |
