...
- [5.1.2 데이터베이스 스키마 상세](#512-데이터베이스-스키마-상세)
  ...
  - [wip_items 테이블](#wip_items-테이블)
  - [wip_process_history 테이블](#wip_process_history-테이블)
...
erDiagram
    ...
    LOTS ||--o{ WIP_ITEMS : has
    WIP_ITEMS ||--o{ WIP_PROCESS_HISTORY : tracks
    WIP_ITEMS ||--o| SERIALS : converts_to
    PROCESSES ||--o{ WIP_PROCESS_HISTORY : executes
    EQUIPMENT ||--o{ WIP_PROCESS_HISTORY : uses
    USERS ||--o{ WIP_PROCESS_HISTORY : operates
...
**용도**: 공정 1~6에서 사용하는 WIP (Work In Progress) Item 관리
...
CREATE TABLE wip_items (
    ...
    CONSTRAINT uk_wip_lot_sequence UNIQUE (lot_id, sequence_in_lot),
    CONSTRAINT chk_wip_status CHECK (status IN ('CREATED', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'CONVERTED')),
    CONSTRAINT chk_sequence_range CHECK (sequence_in_lot BETWEEN 1 AND 100)
);

CREATE INDEX idx_wip_lot_id ON wip_items(lot_id);
CREATE INDEX idx_wip_status ON wip_items(status);
CREATE INDEX idx_wip_serial_id ON wip_items(serial_id);
```

**컬럼 설명**:

- `id`: WIP Item 고유 식별자 (Primary Key)
- `wip_id`: WIP ID 문자열 (예: WIP-KR01PSA2511-001), Unique
- `lot_id`: 소속 LOT (Foreign Key → lots.id)
- `sequence_in_lot`: LOT 내 순번 (1~100)
- `status`: WIP 상태 (CREATED, IN_PROGRESS, COMPLETED, FAILED, CONVERTED)
- `current_process_id`: 현재 진행 중인 공정 (Foreign Key → processes.id), nullable
- `serial_id`: 전환된 Serial (Foreign Key → serials.id), nullable
- `created_at`: WIP 생성 시각
- `converted_at`: Serial로 전환된 시각, nullable

**제약 조건**:

- `uk_wip_lot_sequence`: LOT 내 순번 중복 방지
- `chk_wip_status`: 상태 값 제한
- `chk_sequence_range`: 순번 범위 제한 (1~100)

**인덱스**:

- `idx_wip_lot_id`: LOT별 WIP 조회 성능 향상
- `idx_wip_status`: 상태별 WIP 조회 성능 향상
- `idx_wip_serial_id`: Serial 전환 조회 성능 향상

### wip_process_history 테이블

**용도**: WIP Item의 공정 실행 이력 기록
...
CREATE TABLE wip_process_history (
    ...
    CONSTRAINT chk_result CHECK (result IN ('PASS', 'FAIL', 'REWORK'))
);

CREATE INDEX idx_wip_history_wip_id ON wip_process_history(wip_item_id);
CREATE INDEX idx_wip_history_process_id ON wip_process_history(process_id);
CREATE INDEX idx_wip_history_result ON wip_process_history(result);
CREATE INDEX idx_wip_history_created_at ON wip_process_history(created_at);
```

**컬럼 설명**:

- `id`: 이력 레코드 고유 식별자
- `wip_item_id`: WIP Item (Foreign Key → wip_items.id)
- `process_id`: 실행 공정 (Foreign Key → processes.id)
- `operator_id`: 작업자 (Foreign Key → users.id)
- `equipment_id`: 사용 설비 (Foreign Key → equipment.id), nullable
- `result`: 공정 결과 (PASS, FAIL, REWORK)
- `measurements`: 측정 데이터 (JSONB), nullable
- `defects`: 불량 정보 (JSONB), nullable
- `notes`: 비고, nullable
- `started_at`: 공정 시작 시각
- `completed_at`: 공정 완료 시각, nullable
- `duration_seconds`: 소요 시간 (초), nullable
- `created_at`: 레코드 생성 시각

**제약 조건**:

- `chk_result`: 결과 값 제한 (PASS, FAIL, REWORK)

**인덱스**:

- `idx_wip_history_wip_id`: WIP별 이력 조회
- `idx_wip_history_process_id`: 공정별 이력 조회
- `idx_wip_history_result`: 결과별 통계
- `idx_wip_history_created_at`: 시계열 조회
...
CREATE TABLE production_lines (
    ...
    cycle_time_sec INTEGER CHECK (cycle_time_sec > 0),
    is_active BOOLEAN DEFAULT TRUE,
...
- `wip_id`: WIP ID 문자열 (예: WIP-KR01PSA2511-001) (UK)
- `lot_id`: 소속 LOT FK
- `sequence_in_lot`: LOT 내 순번 (1~100)
- `status`: CREATED, IN_PROGRESS, COMPLETED, FAILED, CONVERTED
- `current_process_id`: 현재 진행 중인 공정 FK (nullable)
- `serial_id`: 전환된 Serial FK (nullable)

**관계:**

- LOT에서 생성되어 공정 1~6 진행
- 공정 7 시작 시점에 Serial로 전환
- WIP Process History로 공정 이력 추적


### WIP 공정 이력 (wip_process_history)

**용도:** WIP Item의 공정 실행 이력 기록

**주요 필드:**

- `wip_item_id`: WIP Item FK
- `process_id`: 실행 공정 FK
- `operator_id`: 작업자 FK
- `equipment_id`: 사용 설비 FK (nullable)
- `result`: 공정 결과 (PASS, FAIL, REWORK)
- `measurements`: 측정 데이터 (JSONB)
- `defects`: 불량 정보 (JSONB)
- `started_at`: 공정 시작 시각
- `completed_at`: 공정 완료 시각

**JSONB 예시 (measurements)**:
...
```json
{
  "defect_codes": ["D001", "D005"],
  "defect_descriptions": ["표면 스크래치", "접착 불량"],
  "severity": "MINOR"
}
```
...
- `line_code`: 생산 라인 코드 (예: KR01, KR02) - 국가코드(2자리)+라인번호(2자리) 형식
- `line_name_ko`: 라인명 (한글)
- `line_name_en`: 라인명 (영문)
- `location`: 시설 내 물리적 위치
- `cycle_time_sec`: 단위당 생산 사이클 시간 (초)
  - 예: 3600초 = 1시간에 1개 생산 (시간당 1개)
  - 예: 7200초 = 2시간에 1개 생산 (시간당 0.5개)
