...
| **REWORK** | 공정 실패 후 재작업을 허용하는 프로세스. WIP 상태를 IN_PROGRESS로 유지하며 무제한 재시도 가능. 최종적으로 PASS 또는 FAIL로 완료 필요 (참조: FR-WIP-004) |
| **LMA** | Linear Muscle Actuator - 로봇의 핵심 구동기 |
| **SMA 스프링** | Shape Memory Alloy 스프링 - 온도로 형상 기억하여 구동 |
| **WIP (Work In Progress)** | 공정 1~6에서 사용하는 작업 중인 제품의 임시 식별자. 형식: `WIP-{LOT}-{SEQ:03d}` (예: WIP-KR01PSA2511-001). 공정 7에서 Serial Number로 전환됨 (참조: Section 2.6.3, Section 5.3.3) |
| **WIP ID** | WIP (Work In Progress)의 19자리 고유 식별자. 구성: 접두어(WIP-) + LOT 번호(11자) + 구분자(-) + 순번(3자). 범위: LOT당 001~100 (참조: Section 2.6.3, Section 5.3.3) |
| **WIP Item** | WIP ID로 추적되는 개별 제품 단위. 상태: CREATED, IN_PROGRESS, COMPLETED, FAILED, CONVERTED (참조: wip_items 테이블) |
| **공정 전환 (WIP → Serial)** | 공정 7 시작 시 WIP ID를 Serial Number로 전환하는 프로세스. 조건: 공정 1~6이 모두 PASS 상태여야 함 (BR-005). 결과: Serial Number 생성 및 WIP 상태 CONVERTED로 변경 (참조: FR-WIP-003, Section 3.2.3.4) |
...
> **이전 섹션:** [6. 투자 계획](./06-investment.md)
