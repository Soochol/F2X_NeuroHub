...
- LOT number format: `{MODEL_CODE}-KR-YYMMDD{D|N}-nnn` (e.g., `PSA10-KR-YYMMDD{D|N}-nnn`)
...
BEGIN
    ...
    INTO v_sequence
    FROM lots
    WHERE lot_number LIKE v_model_code || '-KR-' || v_date_part || v_shift_part || '-%';

    -- Generate LOT number: PSA10-KR-251110D-001
    v_new_lot_number := v_model_code || '-KR-' || v_date_part || v_shift_part || '-' ||
...
1. **LOT Number Format**:
   - Pattern: `{MODEL_CODE}-KR-YYMMDD{D|N}-nnn`
   - Example: `PSA10-KR-251110D-001` (PSA10 model, 2025-11-10, Day shift, sequence 001)
...
BEGIN
    ...
    -- Generate serial number: PSA10-KR-251110D-001-0001
