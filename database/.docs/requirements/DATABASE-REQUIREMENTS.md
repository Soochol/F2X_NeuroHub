...
| `wip_items` | WIP 항목 | WIP 진행 추적 (LOT당 최대 100개) | ~5M/year | P0 |
| `wip_process_history` | WIP 공정 이력 | WIP 상태 변화 및 공정 이력 | ~40M/year | P0 |
...
- **LOT 번호 포맷**: `Country(2)+Line(2)+Model(3)+Month(4)+Seq(3)` (14자)
  - 예: `KR01PSA2511001`
  - KR: 국가코드 (2자)
  - 01: 라인번호 (2자리, 01-99)
  - PSA: 모델코드 약어 (3자)
...
BEGIN
    ...
    -- Get next sequence for production month
    SELECT COALESCE(MAX(
        CAST(SUBSTRING(lot_number FROM 12 FOR 3) AS INTEGER)
    ), 0) + 1 INTO v_sequence
    FROM lots
    WHERE lot_number LIKE '______' || SUBSTRING(v_date_part FROM 1 FOR 4) || '%';

    -- Generate: KR01PSA2511001
    NEW.lot_number := 'KR01' || 'PSA' || SUBSTRING(v_date_part FROM 1 FOR 4) ||
...
- **시리얼 번호 포맷**: `LOT_NUMBER+XXXX` (18자)
  - 예: `KR01PSA25110010001`
...
### 3.8 wip_items (WIP 항목)

**목적**: LOT 또는 개별 Serial 단위의 제조 진행 상태를 추적하는 Work-In-Progress 항목 테이블

**스키마**:
| 컬럼명 | 타입 | Null | 기본값 | 설명 |
|--------|------|------|--------|------|
| id | BIGSERIAL | NOT NULL | AUTO | PK |
| wip_id | VARCHAR(50) | NOT NULL | AUTO | 고유 WIP 식별자 (WIP-{LOT}-{SEQ:03d}) |
| lot_id | BIGINT | NOT NULL | - | FK: lots.id |
| serial_id | BIGINT | NULL | NULL | FK: serials.id (선택사항) |
| status | VARCHAR(20) | NOT NULL | 'CREATED' | 상태: CREATED/IN_PROCESS/PASSED/FAILED |
...
CREATE INDEX idx_wip_items_serial ON wip_items(serial_id) WHERE serial_id IS NOT NULL;
...
**비즈니스 규칙**:
- WIP ID 포맷: `WIP-{LOT_NUMBER}-{SEQ:03d}` (19자)
- LOT당 최대 100개 제한 (트리거로 검증)
- 상태 전이: CREATED → IN_PROCESS → PASSED/FAILED
- lot_id는 필수, serial_id는 선택사항

---

### 3.9 wip_process_history (WIP 공정 이력)

**목적**: WIP 항목의 제조 공정 진행 상태 변화를 기록하는 감사 추적 테이블

**스키마**:
| 컬럼명 | 타입 | Null | 기본값 | 설명 |
|--------|------|------|--------|------|
| id | BIGSERIAL | NOT NULL | AUTO | PK |
| wip_item_id | BIGINT | NOT NULL | - | FK: wip_items.id |
| process_id | BIGINT | NOT NULL | - | FK: processes.id |
| status | VARCHAR(20) | NOT NULL | - | 공정 상태 |
| result | VARCHAR(10) | NOT NULL | - | 결과: PASS/FAIL/REWORK |
...
**인덱스**:
```sql
CREATE INDEX idx_wip_process_history_wip_item ON wip_process_history(wip_item_id);
CREATE INDEX idx_wip_process_history_process ON wip_process_history(process_id);
...
**비즈니스 규칙**:
- 완전한 감사 추적 제공 (모든 WIP 상태 변화 기록)
- 공정별 PASS/FAIL/REWORK 결과 추적
- 이력 레코드는 불변 (append-only)
- 규정 준수를 위한 히스토리 보존
...
| equipment | process_data | equipment_id | 1:N | SET NULL |
...
| process_data | idx_process_data_equipment | Partial | 장비별 공정 조회 | - |
| process_data | idx_process_data_equipment_utilization | Composite | 장비 가동률 분석 | - |
...
| BR-008 | WIP 생성 시 자동 번호 생성 | WIP-{LOT}-{SEQ:03d} 포맷 자동 생성 |
| BR-009 | WIP 상태 전이 검증 | CREATED → IN_PROCESS → PASSED/FAILED |
| BR-010 | LOT당 WIP 최대 100개 제한 | 착공 시 capacity check 수행 |
...
-- 4. WIP 추적 테이블 (wip_items, wip_process_history)
--    - WIP 항목 및 공정 이력 추적
