...
- **11 Core Entities**: Product Models, LOTs, Serials, Processes, Process Data, Users, Audit Logs, Production Lines, Equipment, WIP Items, WIP Process History
- **9 Primary Relationships**: Hierarchical production tracking with process execution records, production line management, equipment tracking, and WIP lifecycle tracking
...
erDiagram
    ...
    lots ||--o{ wip_items : "creates_wip"
    wip_items ||--o{ wip_process_history : "tracks_progress"
    serials ||--o{ wip_items : "links_to"
    ...
    wip_items {
        ...
        BIGINT lot_id FK
        BIGINT serial_id FK "선택사항"
        VARCHAR status "CREATED/IN_PROCESS/PASSED/FAILED"
        TIMESTAMP created_at
        TIMESTAMP updated_at
        TIMESTAMP completed_at
    }
...
**Business Logic**:
- Serial numbers are auto-generated in format: LOT_NUMBER+XXXX (18 chars, e.g., KR01PSA25110010001)
...
## 9. lots → wip_items (1:N)

**Cardinality**: 하나의 LOT는 최대 100개의 WIP 항목을 생성할 수 있습니다.

**Description**: WIP 항목은 배치 단위의 제조 진행 상태를 추적하며, Serial 단위 또는 집계된 추적을 지원합니다. LOT당 최대 100개로 제한됩니다.

**Business Logic**:
- WIP-{LOT_NUMBER}-{SEQ:03d} 포맷으로 자동 생성
- 상태 전이: CREATED → IN_PROCESS → PASSED/FAILED
- LOT당 최대 100개 (트리거로 강제)

---

## 10. wip_items → wip_process_history (1:N)

**Cardinality**: 하나의 WIP 항목은 여러 상태 전이 기록을 가집니다.

**Description**: WIP 항목이 제조 공정을 거치면서 발생하는 모든 상태 변화를 완전하게 감사 추적합니다.

**Business Logic**:
- 모든 상태 변화 기록
- 공정별 결과 캡처 (PASS/FAIL/REWORK)
- 불변 이력 레코드 (append-only)
- 시계열 데이터 분석 지원
...
### 3. WIP Tracking
- **WIP Tracking**: LOT 단위 진행 상태 추적 (최대 100개/LOT)
