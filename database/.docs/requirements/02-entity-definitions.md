...
This document provides comprehensive definitions for all 9 core entities in the F2X NeuroHub MES database. Each entity definition includes:
...
8. [wip_items (WIP 항목)](#28-wip_items-wip-항목)
9. [wip_process_history (WIP 공정 이력)](#29-wip_process_history-wip-공정-이력)
...
- LOT number format: `Country(2)+Line(2)+Model(3)+Month(4)+Seq(3)` (14 chars, e.g., `KR01PSA2511001`)
  - KR: Country code (2 chars)
  - 01: Line number (2 digits, 01-99)
  - PSA: Model code abbreviation (3 chars)
  - 2511: Production month YYMM (4 digits)
...
1. **LOT Number Format**:
   - Pattern: `Country(2)+Line(2)+Model(3)+Month(4)+Seq(3)` (14 chars)
   - Example: `KR01PSA2511001` (KR country, 01 line, PSA model, 2511 month, 001 sequence)
...
- Serial number format: `LOT_NUMBER+XXXX` (18 chars total, XXXX = 0001-0100)
...
BEGIN
    ...
    -- Generate serial number: KR01PSA25110010001 (LOT + 4-digit sequence)
...
1. **Serial Number Format**:
   - Pattern: `LOT_NUMBER+XXXX` (18 chars total)
...
| equipment_id | BIGINT | NULL | NULL | Foreign key to equipment (used for this process) |
...
-- Equipment tracking
CREATE INDEX idx_process_data_equipment
ON process_data(equipment_id)
WHERE equipment_id IS NOT NULL;

-- Equipment utilization analysis
CREATE INDEX idx_process_data_equipment_utilization
...
## 2.8 wip_items (WIP 항목)

### Purpose
LOT 또는 개별 Serial 단위의 제조 진행 상태를 추적하는 Work-In-Progress 항목. 실시간 제조 진행 상태 모니터링을 가능하게 합니다.

### Business Context
- WIP-{LOT}-{SEQ:03d} 포맷 (19자, 예: WIP-KR01PSA2511-001)
- 공정 진행 상태를 단위별로 추적
- LOT당 최대 100개 제한 (제약조건으로 강제)
- process_data의 batch-level 추적 대안
...
| wip_id | VARCHAR(50) | NOT NULL | AUTO | 고유 WIP 식별자 |
| lot_id | BIGINT | NOT NULL | - | FK: lots |
| serial_id | BIGINT | NULL | NULL | FK: serials (선택사항) |
| status | VARCHAR(20) | NOT NULL | 'CREATED' | CREATED/IN_PROCESS/PASSED/FAILED |
| created_at | TIMESTAMP WITH TIME ZONE | NOT NULL | NOW() | 생성 시각 |
| updated_at | TIMESTAMP WITH TIME ZONE | NOT NULL | NOW() | 최종 수정 시각 |
| completed_at | TIMESTAMP WITH TIME ZONE | NULL | NULL | 완료 시각 |

### Constraints

```sql
ALTER TABLE wip_items ADD CONSTRAINT pk_wip_items PRIMARY KEY (id);
ALTER TABLE wip_items ADD CONSTRAINT uk_wip_items_wip_id UNIQUE (wip_id);
ALTER TABLE wip_items ADD CONSTRAINT uk_wip_items_lot_serial
  UNIQUE (lot_id, serial_id) WHERE serial_id IS NOT NULL;
ALTER TABLE wip_items ADD CONSTRAINT fk_wip_items_lot
  FOREIGN KEY (lot_id) REFERENCES lots(id) ON DELETE RESTRICT;
ALTER TABLE wip_items ADD CONSTRAINT fk_wip_items_serial
  FOREIGN KEY (serial_id) REFERENCES serials(id) ON DELETE SET NULL;
ALTER TABLE wip_items ADD CONSTRAINT chk_wip_items_status
  CHECK (status IN ('CREATED', 'IN_PROCESS', 'PASSED', 'FAILED'));
```

### Indexes

```sql
CREATE INDEX idx_wip_items_lot ON wip_items(lot_id);
CREATE INDEX idx_wip_items_serial ON wip_items(serial_id)
  WHERE serial_id IS NOT NULL;
CREATE INDEX idx_wip_items_status ON wip_items(status);
CREATE INDEX idx_wip_items_active ON wip_items(lot_id, status)
  WHERE status IN ('CREATED', 'IN_PROCESS');
...
CREATE TRIGGER trg_wip_items_generate_id
  BEFORE INSERT ON wip_items FOR EACH ROW
  EXECUTE FUNCTION generate_wip_id();
CREATE TRIGGER trg_wip_items_validate_capacity
  BEFORE INSERT ON wip_items FOR EACH ROW
  EXECUTE FUNCTION validate_wip_capacity();
CREATE TRIGGER trg_wip_items_updated_at
  BEFORE UPDATE ON wip_items FOR EACH ROW
  EXECUTE FUNCTION update_timestamp();
CREATE TRIGGER trg_wip_items_audit
  AFTER INSERT OR UPDATE OR DELETE ON wip_items FOR EACH ROW
  EXECUTE FUNCTION log_audit_event();
```

### Trigger Function: generate_wip_id()
...
DECLARE
    ...
    v_sequence INTEGER;
BEGIN
    SELECT lot_number INTO v_lot_number FROM lots WHERE id = NEW.lot_id;
    SELECT COALESCE(MAX(CAST(SUBSTRING(wip_id FROM '.*-(\d+)$') AS INTEGER)), 0) + 1
    INTO v_sequence FROM wip_items WHERE lot_id = NEW.lot_id;
    NEW.wip_id := 'WIP-' || v_lot_number || '-' || LPAD(v_sequence::TEXT, 3, '0');
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
```
...
1. **WIP ID Format**: WIP-{LOT_NUMBER}-{SEQ:03d} (19 chars)
2. **Capacity**: LOT당 최대 100개 (트리거로 검증)
3. **Status Transitions**: CREATED → IN_PROCESS → PASSED/FAILED
4. **Relationships**:
   - lot_id 필수 연결
   - serial_id 선택적 연결
   - wip_process_history로 추적

---

## 2.9 wip_process_history (WIP 공정 이력)

### Purpose
WIP 항목의 제조 공정 진행 이력을 완전하게 감사 추적하는 테이블. 상태 변화 및 공정 결과를 기록합니다.
...
|--------|------|------|---------|-------------|
| id | BIGSERIAL | NOT NULL | AUTO | PK |
| wip_item_id | BIGINT | NOT NULL | - | FK: wip_items |
| process_id | BIGINT | NOT NULL | - | FK: processes |
| status | VARCHAR(20) | NOT NULL | - | 공정 상태 |
| result | VARCHAR(10) | NOT NULL | - | PASS/FAIL/REWORK |
| started_at | TIMESTAMP WITH TIME ZONE | NOT NULL | - | 공정 시작 시각 |
| completed_at | TIMESTAMP WITH TIME ZONE | NULL | NULL | 공정 완료 시각 |
| notes | TEXT | NULL | NULL | 추가 메모 |
| created_at | TIMESTAMP WITH TIME ZONE | NOT NULL | NOW() | 레코드 생성 시각 |

### Constraints

```sql
ALTER TABLE wip_process_history ADD CONSTRAINT pk_wip_process_history PRIMARY KEY (id);
ALTER TABLE wip_process_history ADD CONSTRAINT fk_wip_process_history_wip_item
  FOREIGN KEY (wip_item_id) REFERENCES wip_items(id) ON DELETE RESTRICT;
ALTER TABLE wip_process_history ADD CONSTRAINT fk_wip_process_history_process
  FOREIGN KEY (process_id) REFERENCES processes(id) ON DELETE RESTRICT;
ALTER TABLE wip_process_history ADD CONSTRAINT chk_wip_process_history_result
  CHECK (result IN ('PASS', 'FAIL', 'REWORK'));
```
...
```sql
CREATE INDEX idx_wip_process_history_wip_item
  ON wip_process_history(wip_item_id);
CREATE INDEX idx_wip_process_history_process
  ON wip_process_history(process_id);
CREATE INDEX idx_wip_process_history_composite
  ON wip_process_history(wip_item_id, process_id, created_at);
CREATE INDEX idx_wip_process_history_result
  ON wip_process_history(result, created_at);
```
...
CREATE TRIGGER trg_wip_process_history_audit
  AFTER INSERT OR UPDATE OR DELETE ON wip_process_history FOR EACH ROW
  EXECUTE FUNCTION log_audit_event();
```

### Business Rules

1. **Complete Audit Trail**: 모든 WIP 상태 변화 기록
2. **Process Tracking**: processes 테이블과 연계
3. **Result Capture**: 각 단계별 PASS/FAIL/REWORK 추적
4. **Immutability**: 규정 준수를 위한 이력 레코드 보존
...
This document defines all 9 core entities for the F2X NeuroHub MES database with:
