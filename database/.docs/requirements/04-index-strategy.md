...
### wip_items 테이블 (6개) ⭐ **NEW**

| 인덱스명 | 타입 | 컬럼 | 목적 | 예상 효과 |
|---------|------|------|------|-----------|
| pk_wip_items | B-Tree | id | Primary Key | - |
| idx_wip_items_lot | B-Tree | lot_id | LOT별 WIP 조회 | 100ms → 5ms |
| idx_wip_items_serial | B-Tree | serial_id | Serial별 WIP 조회 | 100ms → 5ms |
| idx_wip_items_status | B-Tree | status | 상태별 WIP 조회 | 200ms → 10ms |
| idx_wip_items_active | Composite | lot_id, status WHERE status IN ('CREATED', 'IN_PROCESS') | 활성 WIP 조회 | 300ms → 10ms |
| idx_wip_items_completed_at | B-Tree | completed_at WHERE completed_at IS NOT NULL | 완료된 WIP 조회 | 200ms → 10ms |

**WIP 쿼리 예시**:
```sql
-- LOT의 활성 WIP 조회
SELECT wip_id, status FROM wip_items
WHERE lot_id = 1 AND status IN ('CREATED', 'IN_PROCESS')
ORDER BY wip_id;
-- Execution time: 10ms (before: 300ms)
```

---

### wip_process_history 테이블 (4개) ⭐ **NEW**

| 인덱스명 | 타입 | 컬럼 | 목적 | 예상 효과 |
|---------|------|------|------|-----------|
| pk_wip_process_history | B-Tree | id | Primary Key | - |
| idx_wip_process_history_wip_item | B-Tree | wip_item_id | WIP 항목별 조회 | 300ms → 10ms |
| idx_wip_process_history_process | B-Tree | process_id | 공정별 조회 | 400ms → 15ms |
| idx_wip_process_history_composite | Composite | wip_item_id, process_id, created_at | WIP 공정 이력 조회 | 500ms → 20ms |
...
- [05-migration-plan.md](./05-migration-plan.md) - 마이그레이션 계획
