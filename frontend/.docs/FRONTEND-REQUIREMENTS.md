...
##### 착공 API (POST /api/v1/process-operations/start)

착공 등록 시 다음 파라미터를 전송합니다:

- `serial_number`: 시리얼 번호 (필수)
- `process_id`: 공정 ID (필수)
- `worker_id`: 작업자 ID (필수)
- `equipment_id`: 설비 코드 (예: "LASER-001")
- `line_id`: 생산 라인 코드 (예: "LINE-A")
...
├─────────────────────────────────────────────┤
│  시리얼 정보                                │
│  - 시리얼 번호: KR01PSA25110010001         │
│  - LOT 번호: KR01PSA2511001                │
│  - 현재 상태: IN_PROGRESS                   │
├─────────────────────────────────────────────┤
│  공정별 데이터 입력 폼                      │
│  (공정에 따라 동적으로 변경)                │
...
#### 스캔 포맷
- 시리얼 번호: `{line}{model}{YYMM}{seq}{serial_seq}` (18자리, 예: KR01PSA25110010001)
- LOT 번호: `{line}{model}{YYMM}{seq}` (14자리, 예: KR01PSA2511001)
...
- [03-1-functional.md](../../docs/user-specification/03-requirements/03-1-functional.md) - 기능 요구사항
- [03-2-api-specs.md](../../docs/user-specification/03-requirements/03-2-api-specs.md) - API 명세
- [04-3-tech-stack.md](../../docs/user-specification/04-architecture/04-3-tech-stack.md) - 기술 스택