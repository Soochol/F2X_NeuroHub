...
import { LotStatus, type Lot, type LotUpdate } from '@/types/api';
...
export const LotDetailModal = ({ isOpen, onClose, lotId, onUpdate }: LotDetailModalProps) => {
  ...
  const fetchLot = async () => {
    ...
    try {
      ...
      setError(err.message || 'LOT 정보를 불러오지 못했습니다');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleUpdate = async () => {
    try {
      ...
      setError(err.message || 'LOT 업데이트 실패');
    }
  };
  ...
  if (error && !lot) {
    return (
      <Modal isOpen={isOpen} onClose={onClose} title="LOT 상세 정보">
        <div style={{ color: '#e74c3c', padding: '20px' }}>{error}</div>
      </Modal>
    );
  }
      ...
      <div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            ...
            <div>
              <div style={{ fontSize: '12px', color: '#7f8c8d', marginBottom: '5px' }}>목표 수량</div>
        ...
        <div style={{ marginBottom: '20px', paddingTop: '20px', borderTop: '1px solid #e0e0e0' }}>
          ...
          {isEditing ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div>
                <div style={{ fontSize: '12px', color: '#7f8c8d', marginBottom: '5px' }}>Busbar</div>
                <div>{lot.busbar_lot || 'N/A'}</div>
              </div>
              <div>
                <div style={{ fontSize: '12px', color: '#7f8c8d', marginBottom: '5px' }}>SMA Spring</div>
                <div>{lot.sma_spring_lot || 'N/A'}</div>
              </div>
              <div>
                <div style={{ fontSize: '12px', color: '#7f8c8d', marginBottom: '5px' }}>Pin</div>
                <div>{lot.pin_lot || 'N/A'}</div>
              </div>
              <div>
                <div style={{ fontSize: '12px', color: '#7f8c8d', marginBottom: '5px' }}>Housing</div>
        ...
        <div style={{ paddingTop: '20px', borderTop: '1px solid #e0e0e0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', fontSize: '13px' }}>
            <div>
              <div style={{ color: '#7f8c8d', marginBottom: '5px' }}>생성일시</div>
              <div>{format(new Date(lot.created_at), 'yyyy-MM-dd HH:mm:ss')}</div>
            </div>
            <div>
              <div style={{ color: '#7f8c8d', marginBottom: '5px' }}>수정일시</div>
              <div>{format(new Date(lot.updated_at), 'yyyy-MM-dd HH:mm:ss')}</div>
            </div>
            {lot.completed_at && (
              <div>
                <div style={{ color: '#7f8c8d', marginBottom: '5px' }}>완료일시</div>
                <div>{format(new Date(lot.completed_at), 'yyyy-MM-dd HH:mm:ss')}</div>
              </div>
            )}
            {lot.closed_at && (
              <div>
                <div style={{ color: '#7f8c8d', marginBottom: '5px' }}>종료일시</div>
