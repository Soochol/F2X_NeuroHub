...
loading, and validating sequence packages.
"""
...
from station_service.sequence.manifest import HardwareDefinition, SequenceManifest
    ...
    async def load_sequence_class(
        ...
        try:
            ...
            try:
                ...
                parent_module_name = f"_sequence_packages"
        ...
        # Validate that it has sequence metadata
        if get_sequence_meta(sequence_class) is None:
            logger.warning(
                f"Class '{class_name}' is not decorated with @sequence decorator"
            )
